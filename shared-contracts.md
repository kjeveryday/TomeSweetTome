...
- The active creature is the hero the scene renders: `ActiveCreatureChanged` projects the incoming creature's look and name onto the v1 `creature`/`name` fields as well as its care state onto the root care fields. The first activation of a hatched native game (no prior active creature) adopts the accumulated root care into the newly active creature rather than resetting it — v1 book-apply semantics, never-punish.
- **Care scope (frozen, package 5 — "gentle collection"):** Inactive (non-active) creatures pause. Their `careState` snapshot is frozen while archived — no drift, decay, absence, or catch-up — and reactivation never applies retroactive absence for the inactive interval. The daily care cap is per-creature (`careState.actionsToday`/`dayKey`), not player-wide.
- **Relationship development (frozen, package 5):** A creature records at most one relationship day per work per local date in `CreatureRecord.relationshipDayKeys` (unique, deduped). The first-reading reveal day is the creature's first relationship day; each subsequent distinct reading day advances to the next configured non-ranked response (`content.json` `relationship.responses`, cyclic, no strength). Finishing a book sets `CreatureRecord.finished: true` permanently — never removed by a later reread or paused/not-for-me status. Relationship days and the finish marker grant no CP, stage, care power, or reading progress and never change absence behavior.
